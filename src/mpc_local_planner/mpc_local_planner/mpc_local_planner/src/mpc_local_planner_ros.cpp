...
bool MpcLocalPlannerROS::setPlan(const std::vector<geometry_msgs::PoseStamped>& orig_global_plan)
{
    ...
    ros::NodeHandle nh;
    bool use_local_planner_only = false;
    nh.param("/move_base/MpcLocalPlannerROS/controller/use_local_planner_only", use_local_planner_only, use_local_planner_only);

    // store the global plan
    _global_plan.clear();
    _global_plan = orig_global_plan;

    if (use_local_planner_only) {
        ...
        _global_plan = _tmp_global_plan;


        for (int path; path < _global_plan.size(); path++) {
            std::cout << "Global Trajectory " << _global_plan[path].pose << std::endl;
        }
    }
                                                     ...
                                                     geometry_msgs::TwistStamped& cmd_vel, std::string& message)
{
...

    {
        ...
        // std::cout << "Transformed Global Plan " << transformed_plan << std::endl;
    ...
    // NHH shift rolling_window map center, 20231109, shift to the direction of moving
    if (cmd_vel.twist.linear.x < -0.001){	// 0.001 instead of 0 to avoid fluctuation
    	_costmap_ros->getLayeredCostmap()->isRobotBackward_ = true;
    }else if (cmd_vel.twist.linear.x > 0.001){
    	_costmap_ros->getLayeredCostmap()->isRobotBackward_ = false;
    }
//    printf("Twist message: %.3s
    ROS_INFO("Speed: %.3f,\t\tSteering: %.3f\n", cmd_vel.twist.linear.x, cmd_vel.twist.angular.z);
    // std::cout << "cmd_vel Speed (m/s) " << cmd_vel.twist.linear.x << std::endl;
    // std::cout << "cmd_vel Steering (m/s) " << cmd_vel.twist.angular.z << std::endl;
                                             ...
                                             geometry_msgs::TransformStamped* tf_plan_to_global) const
{
    ...
    try
    {
    	...
    	// NHH shift rolling_window map center, 20231108
        geometry_msgs::PoseStamped costmap_center_pose = robot_pose;
        costmap_center_pose.pose.position.x = costmap.getOriginX() + costmap.getSizeInMetersX()/2;
        costmap_center_pose.pose.position.y = costmap.getOriginY() + costmap.getSizeInMetersY()/2;
        geometry_msgs::PoseStamped costmap_center_pose_in_plan_frame;
        tf.transform(costmap_center_pose, costmap_center_pose_in_plan_frame, plan_pose.header.frame_id);
...
//            double y_diff      = robot_pose.pose.position.y - global_plan[j].pose.position.y;
            double x_diff      = costmap_center_pose_in_plan_frame.pose.position.x - global_plan[j].pose.position.x;
            double y_diff      = costmap_center_pose_in_plan_frame.pose.position.y - global_plan[j].pose.position.y;
...
//            double y_diff = robot_pose.pose.position.y - global_plan[i].pose.position.y;
            double x_diff = costmap_center_pose_in_plan_frame.pose.position.x - global_plan[i].pose.position.x;
            double y_diff = costmap_center_pose_in_plan_frame.pose.position.y - global_plan[i].pose.position.y;
...
{
    std::lock_guard<std::mutex> l(_custom_obst_mutex);
    // std::cout << "Obstacle message " << *obst_msg << std::endl;
