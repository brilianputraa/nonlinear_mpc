                           ...
                           teb_local_planner::RobotFootprintModelPtr robot_model, const std::vector<teb_local_planner::PoseSE2>& via_points)
{
    ...
    _ctrl_pub = nh.advertise<ackermann_msgs::AckermannDriveStamped>("/rbcar_robot_control/command", 1, false);
    ...
    std::cout << "PUB OCP " << _publish_ocp_results << std::endl;
                      ...
                      corbo::TimeSeries::Ptr u_seq, corbo::TimeSeries::Ptr x_seq)
{
    ...
    //{
      //  std::cout << "Transformed Global Plan " << new_plan[i].pose << std::endl;
    //} 

    // ROS_INFO("Path [%s]", initial_plan.pose);

    // Drop the whole global path except the last and the first
    // std::vector<geometry_msgs::PoseStamped> new_plan;
    // std::vector<geometry_msgs::PoseStamped> temp_plan = std::copy(initial_plan);

    // new_plan.push_back(initial_plan[0]);
    // new_plan.push_back(initial_plan[1]);

    // initial_plan.resize(0);
...
void Controller::publishOptimalControlResult()
{
    ...
    ackermann_msg.drive.speed  = new_vel;
    ...
    std::cout << "Distance to goal " << dist << " Control velocity before scaling " << msg.controls[0] << std::endl;
    std::cout << "Control velocity signal " << new_vel << " Control steering signal " << msg.controls[1] << std::endl;
    
    _ocp_result_pub.publish(msg);
}

double Controller::speedControlRegulator(double vel, double dist_to_goal, double shift, double scale)
{
    double new_vel;
    double vel_scaler;

    vel_scaler = 1 / (1 + std::exp(-scale*(dist_to_goal - shift)));
    new_vel = vel * vel_scaler;

    std::cout << "Velocity Scaler " << vel_scaler << std::endl;
...
corbo::DiscretizationGridInterface::Ptr Controller::configureGrid(const ros::NodeHandle& nh)
{
    ...
    if (grid_type == "fd_grid")
    {
        ...
        std::cout << "Collocation method " << collocation_method << std::endl;
